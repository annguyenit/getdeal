...
function writeCalendarContent()
{
	...
	for(var no=1;no<=daysInMonth;no++){
		...
		if(currentYear == inputYear && currentMonth == inputMonth && no==tmpDayd){
			cell.className='activeDay';	
		}else if(currentMonth < inputMonth && no > tmpDayd  && currentYear <= inputYear || currentMonth <= inputMonth && no < tmpDayd  && currentYear <= inputYear || currentMonth <= inputMonth && no == tmpDayd  && currentYear <= inputYear || currentMonth <= inputMonth && no > tmpDayd  && currentYear < inputYear || currentMonth >= inputMonth && no < tmpDayd  && currentYear < inputYear || currentMonth >= inputMonth && no >= tmpDayd  && currentYear < inputYear){
			cell.className='oldDay';
		}
...
function pickDate(e,inputDay)
{
	...
	closeCalendar();
		...
		document.getElementById('dtpid_date').blur()
		{
			showTime(document.getElementById('dtpid_date').value);
		}
...
function test() {
	...
	calendarDiv.style.left = (window.outerWidth-(window.outerWidth/5.5))+ 'px';
	calendarDiv.style.top = getTopPos(inputObj)+5 + 'px';
	calendarDiv.style.marginLeft = "-"+(window.outerWidth/2)+"px";;
	if(iframeObj){
...
{  
	...
	calendarDiv.style.top = getTopPos(inputObj)+7 + 'px';
	calendarDiv.style.marginLeft = - 220 +"px";
	if(iframeObj){
		iframeObj.style.left = calendarDiv.style.left;
		iframeObj.style.top =  calendarDiv.style.top;
	}
...
function displayCalendar(inputField,format,buttonObj)
{
	...
	var tar = (e && e.target) || (e && e.srcElement)
	if(tar.nodeName === 'DIV')
	{
			//closeCalendar();
