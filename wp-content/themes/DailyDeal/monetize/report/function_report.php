...
function saller_cmb($saller_id=''){
	...
	$saller_sql = mysql_query("select DISTINCT u.ID, u.user_nicename from $user_db_table_name u,$post_table p where u.ID = p.post_author and p.post_type = '".CUSTOM_POST_TYPE1."' ");
...
function deal_count_post($author_id){
	global $wpdb;
	$post_table = $wpdb->prefix."posts";
	$deal_cnt = mysql_query("select count(ID) as cnt from $post_table where post_author = '".$author_id."' and post_type='".CUSTOM_POST_TYPE1."' and post_status = 'publish'");
...
function deal_list_perauthor($author_id){
	...
	$post_table = $wpdb->prefix."posts";
	$display_deal = '';
	$deal_cnt = mysql_query("select ID,post_title from $post_table where post_author = '".$author_id."' and post_type='".CUSTOM_POST_TYPE1."' and post_status = 'publish'");
