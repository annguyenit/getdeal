...
<?php  	$dealcnt_sql = $wpdb->get_results("select p.* from $post_db_table_name p where post_author = '".$curauth->ID."' and p.post_type = '".CUSTOM_POST_TYPE1."' and p.post_status = 'publish' limit $strtlimit,$recordsperpage ");
	...
	if(mysql_affected_rows() >0){
			...
			<?php if(get_post_meta($total_deals_obj->ID,'file_name',true) != "") {?>
							<img src="<?php echo templ_thumbimage_filter(get_post_meta($total_deals_obj->ID,'file_name',true),145,160);?>" width="145" height="160" alt="deal image" />
			<?php } else { ?>
