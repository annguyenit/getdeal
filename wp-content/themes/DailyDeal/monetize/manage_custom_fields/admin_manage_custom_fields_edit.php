...
if($_REQUEST['cf'])
{
	 _e('Edit custom field','templatic');	
}else
{
	 _e('Add a new custom field','templatic');
}
?></h3>
<?php
if($_REQUEST['msg']=='success')
{
	$message = __('Saved successfully.','templatic');	
}
?>
<?php if($message){?>
...
</script><p><?php _e('You can add a new custom field here. While submitting a post or a deal, sellers will be required to fill it.','templatic');?></p>
...
<tr>
  <td width="30%"><strong><?php _e('Show in post-type: ','templatic');?></strong></td>
  <td align="left"> <select name="post_type1" id="post_type1"  <?php if($post_val->is_delete=='1'){?> disabled="disabled" <?php }?>>
  <?php
...
<br />    <p><?php _e('Specify whether this custom field is shown while adding/editing a new Post or while adding/editing a new Deal.','templatic');?></p>
  </td>
  </tr>
<tr>
...
<br />    <p><?php _e('Select the type of this custom field.','templatic');?></p>
  ...
  <td align="left"><input type="text" name="option_values" id="option_values" value="<?php echo $post_val->option_values;?>" size="50" />
   <br />    <span><?php _e('Comma seperated values.','templatic');?></span>
  </td>
  </tr>
  <tr id="admin_title_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td width="30%"><strong><?php _e('Field name (for admin only) : ','templatic');?></strong></td>
  <td align="left"><input type="text" name="admin_title" id="admin_title" value="<?php //echo $post_val->admin_title;?>" size="50" />
<br />  <p><?php _e('Name of the field <u>for admin reference only</u>. It won\'t be displayed in the front end.','templatic');?></p>
  </td>
  </tr>
  
  <tr id="admin_label_id"  <?php if($post_val->ctype=='head'){?> style="display:none;" <?php }else{?> style="display:block;" <?php }?>>
  <td ><strong><?php _e('Field label (for back-end): ','templatic');?></strong></td>
  <td align="left"><input type="text" name="clabels" id="clabels" value="<?php echo $post_val->clabels;?>" size="50" />
    <br />    <p><?php _e('The field label for the custom field displayed in the back-end.','templatic');?></p>
  </td>
  </tr>
  
  <tr>
  <td ><strong><?php _e('Field name (for front-end): ','templatic');?></strong></td>
...
<br />    <p><?php _e('The name you provide here will be displayed as the field\'s name (label) in the front-end.','templatic');?></p>
  ...
  <td align="left"><input type="text" name="default_value" id="default_value" value="<?php echo $post_val->default_value;?>" size="50" />
    <br />    <p><?php _e('Enter the default value of the custom field.','templatic');?></p>
  </td>
  </tr>
  <tr>
  ...
  <option value="0" <?php if($post_val->is_active=='0'){ echo 'selected="selected"';}?>><?php _e('No','templatic');?></option>
      ...
      <br />    <p><?php _e('This setting activates/de-activates the custom field in the front-end and the back-end.','templatic');?></p>
 </td>
  </tr>
  ...
  <option value="0" <?php if($post_val->is_require=='0'){ echo 'selected="selected"';}?>><?php _e('No','templatic');?></option>
      ...
      <br />    <p><?php _e('Specify whether or not this field is required to be filled in compulsarily by users.','templatic');?></p>
 </td>
  </tr>
  ...
  <td ><strong><?php _e('Show on detail page? : ','templatic');?></strong></td>
  <td align="left">
  ...
  <option value="0" <?php if($post_val->show_on_detail=='0'){ echo 'selected="selected"';}?>><?php _e('No','templatic');?></option>
      ...
      <br />    <span><?php _e('Specify whether or not this field be shown on the &lsquo;Detail page&rsquo;.','templatic');?></span>
  </td>
  </tr>
    <tr>
  <td >&nbsp;</td>
