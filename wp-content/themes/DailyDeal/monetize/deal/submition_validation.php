...
$js_code = '';
//$js_code .= '//global vars ';
$js_code .= 'var propertyform = jQuery("#postdeal_frm"); 
'; //form Id
$jsfunction = array();
for($i=0;$i<count($validation_info);$i++) {
	...
	if($type == 'multicheckbox' || $type=='checkbox' || $type=='radio')
	{
		...
		function validate_'.$name.'()
		{
			...
			{
				'.$espan.'.text("'.$msg.'");
				'.$espan.'.addClass("error message_error");
				if("'.$name.'" == "category")
				 {
		...
		$js_code .= '
			{
				...
				'.$name.'.addClass("errortextfield");
				'.$espan.'.text("Please Enter Some Text");
				'.$espan.'.addClass("error message_error");
				return false;
			}
			else{
				'.$name.'.removeClass("errortextfield");
				'.$name.'.addClass("textfield");
				'.$espan.'.text("");
				'.$espan.'.removeClass("error message_error");
				return true;
			}
		}
		';
