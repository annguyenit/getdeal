...
date_default_timezone_set(get_option('timezone_string'));
	...
	$all_total_deals = mysql_query("select distinct p.ID,p.* from $post_db_table_name p ,$postmeta_db_table_name pm where (p.post_type = '".CUSTOM_POST_TYPE1."' and p.post_status = 'publish') and ((pm.meta_key = 'is_expired' and pm.meta_value = '0') or (pm.meta_key = 'is_expired' and  pm.meta_value = '')) and p.ID = pm.post_id  $sqlsql ORDER BY p.ID DESC ");
	...
	$all_dealcnt_sql = $wpdb->get_results("select distinct p.ID,p.* from $post_db_table_name p ,$postmeta_db_table_name pm where (p.post_type = '".CUSTOM_POST_TYPE1."' and p.post_status = 'publish') and ((pm.meta_key = 'is_expired' and pm.meta_value = '0') or (pm.meta_key = 'is_expired' and  pm.meta_value = '')) and p.ID = pm.post_id  $sqlsql ORDER BY p.ID DESC  limit $strtlimit,$recordsperpage");
...
<div <?php post_class('post posts_deals'); ?> id="livedealm_<?php the_ID(); ?>" > 			   
			...
			<?php 				}?>
							...
							?>
							<script>
							displayTZCountDown(setTZCountDown('<?php echo $tar_date[1]; ?>','<?php echo $tar_date[0]; ?>','<?php echo $tar_time[0]; ?>','<?php echo $tar_time[1]; ?>','<?php echo $tar_time[2]; ?>','<?php echo $timezone; ?>','<?php echo $exp_year[1]; ?>'),'countdowncontainerlive_<?php _e($post->ID,'templatic'); ?>','fr_<?php echo $post->ID; ?>','<?php echo $tardate1; ?>');
							</script>
                            </div>
            <?php 		} ?>
			<?php	}?>
