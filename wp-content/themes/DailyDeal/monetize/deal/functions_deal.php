<?php
	...
	function templ_add_template_buy_deal_page($template)
	{
		...
		}elseif($_GET['ptype'] == 'alle_deals_tab')  // PAYMENT GATEWAY NOTIFY URL
		{
			$template = TEMPL_BUY_DEAL_FOLDER.'alle_deals_tab.php';
		}elseif($_GET['ptype'] == 'live_deal_tab')  // PAYMENT GATEWAY NOTIFY URL
		{
			$template = TEMPL_BUY_DEAL_FOLDER.'live_deal_tab.php';
		}elseif($_GET['ptype'] == 'verlopen_deals_tab')  // PAYMENT GATEWAY NOTIFY URL
		{
			$template = TEMPL_BUY_DEAL_FOLDER.'verlopen_deals_tab.php';
		...
		elseif($_GET['ptype'] == 'taxonomy_alle_deals_tab')  // PAYMENT GATEWAY NOTIFY URL
		{
			
			$template = TEMPL_BUY_DEAL_FOLDER.'taxonomy_alle_deals_tab.php';
		...
		}elseif($_GET['ptype'] == 'taxonomy_verlopen_deals_tab')  // PAYMENT GATEWAY NOTIFY URL
		{
			
			$template = TEMPL_BUY_DEAL_FOLDER.'taxonomy_verlopen_deals_tab.php';
	...
	function display_deal_image($deal_id,$width = '',$height = '') {
		...
		if (count($attachments) > 0) {
			...
			<img src="<?php echo templ_thumbimage_filter($attachment->guid,$width,$height);?>" width="<?php echo $width;?>" height="<?php echo $height; ?>"  alt="" />
			<?php 		}
		}else { ?>
