...
$select_transql = $wpdb->get_row("select * from $transection_db_table_name where trans_id = '".$_GET['pid']."' and status = '1' ");
...
{  
	...
	if($coupon_address!="")
	{
		...
		<p>Trans ID: %s\r</p>  <p>Status: %s\r</p>	<p>Date: %s\r</p>",'templatic'),$transifo['post_id'],$transifo['post_title'],$transifo['deal_coupon'],$coupon_address,$_GET['pid'],'Success',date("Y-m-d",strtotime($transifo['payment_date'])));
	}
	else
	{
		...
		if($tpid > 0)
		{
			...
			<p>".DEAL_TITLE_TEXT.": %s \r</p>
			<p>".DEAL_COUPON.": %s \r	</p>
			<p>Trans ID: %s\r</p>  <p>Status: %s\r</p>	<p>Date: %s\r</p>",'templatic'),$transifo['post_id'],$transifo['post_title'],$transifo['deal_coupon'],$_GET['pid'],'Success',date("Y-m-d",strtotime($transifo['payment_date'])));
		}else{
			$transaction_details = sprintf(__("
			<p>".PAYMENT_DEAL." #%s\r</p>
			<p>".DEAL_TITLE_TEXT.": %s \r</p>
			<p>".DEAL_COUPON.": %s \r	</p>
			<p>".INSTRUCTION."</p>
			<p>Trans ID: %s\r</p>  <p>Status: %s\r</p>	<p>Date: %s\r</p>",'templatic'),$transifo['post_id'],$transifo['post_title'],$_GET['pid'],'Success',date("Y-m-d",strtotime($transifo['payment_date'])));
		}
	}
}
else
{
	...
	if($tpid > 0)
	{
		...
		<p>".DEAL_COUPON.": %s \r	</p>
		<p>".$link.": %s \r	</p>
		<p>Trans ID: %s\r</p> <p>Status: %s\r</p>	<p>Date: %s\r</p>",'templatic'),$transifo['post_id'],$transifo['post_title'],$transifo['deal_coupon'],$post_title_link,$_GET['pid'],'Success',date("Y-m-d",strtotime($transifo['payment_date'])));
	}else{
		$transaction_details = sprintf(__("
		<p>".PAYMENT_DEAL." #%s\r	</p>
		<p>".DEAL_TITLE_TEXT.": %s \r</p>
