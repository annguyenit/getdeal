...
get_header(); ?>
<?php
	if($_SERVER['REMOTE_ADDR'] == '113.162.182.19')
	{
		/*
		$tag_name = 'Dealdonkey';  // tag to 'delete'
		$taxonomy = 'deal-categorie';
		$tag = get_term_by('name',$tag_name, $taxonomy);
		$objects = get_objects_in_term($tag->term_id,$taxonomy);
		if(count($objects))
		{
			foreach ($objects as $id) {
				wp_delete_object_term_relationships( $id, 'deal-categorie');
				wp_set_object_terms($id, 'Verassingen','deal-categorie', true);
			}
		}
		echo "<pre>";
		print_r($objects);
		...
		die('123');
		*/
		//mp_deals_importer_do_nudeal(); die;
		//mp_deals_importer_do_nudeal(); die;
		//mp_fb_poster_hook(); die;
...
$args = array('numberposts' => 1,'meta_query' => array(
		array(
			...
			'key' => 'status',
			'value' => '2',
		)*/),'post_status' => 'publish','post_type' => CUSTOM_POST_TYPE1,/*'meta_key' =>'is_show' , 'meta_value' =>'1' ,*/'orderby' =>'DESC');
...
<!-- top content #end -->
	...
	<div class="box_bg">
	<div class=" box_bottom" style="position: relative">
<?php foreach( $recent_posts as $post ){  // foreach loop BOF
		if(get_post_meta($post->ID,'status',true) == 2) {
			...
			<div class="content_left">
				<div class="mp_relative" style="position: relative">
				<a href="<?php the_permalink(); ?>">
				<?php if(get_post_meta($post->ID,'file_name',true) != "") { ?>
						<img src="<?php echo templ_thumbimage_filter(get_post_meta($post->ID,'file_name',true),330,250); ?>" width="330" height="250" alt="" />
				<?php } else { ?>
						<img src="<?php echo get_template_directory_uri()."/images/no-image.png"; ?>" width="330" height="250" alt="" />
				<?php } ?>
				</a>
				<div class="mp_deal_price" style="top: 165px; right: 83px;">
				...
				<?php if (get_option('ptthemes_kangaroo') == "Yes"){?>
				<div class="mp_kangaroo">
					<img src="<?php echo get_option('ptthemes_kangaroo_image');?>" alt="www.getdeals.nl"/>
				</div>
				<?php } ?>
			</div>
...
<?php 								}?>
							...
							?>
							<script>
							displayTZCountDown(setTZCountDown('<?php echo $tar_date[1]; ?>','<?php echo $tar_date[0]; ?>','<?php echo $tar_time[0]; ?>','<?php echo $tar_time[1]; ?>','<?php echo $tar_time[2]; ?>','<?php echo $timezone; ?>','<?php echo $exp_year[1]; ?>'),'countdowncontainer','fr_index','<?php echo $tardate1; ?>');
							</script>
...
<?php 					} ?></li>
		...
		<?php 		}
				...
				<div class="share_div index_share_spacer" style="padding-left: 300px">
				<span style="float: left; margin-right: 30px;"><strong>Maak kans op een gratis deal!</strong> Deel het met je vrienden!</span>
				<?php /*
