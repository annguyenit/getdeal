...
class onecolumnslist extends WP_Widget {
	...
	// prints the widget
		...
		<h3 class="deal_widget_title"><?php _e($title,'templatic'); ?> <small> ( <a href="<?php echo site_url();?>/?ptype=taxonomy_all_deal_tab" class="b_viewalldeal"><?php _e(VIEW_ALL_DEAL,'templatic');?></a> ) </small> </h3>
...
<?php 						if(get_post_meta($post->ID,'file_name',true) != "") { ?>
								<img src="<?php echo templ_thumbimage_filter(get_post_meta($post->ID,'file_name',true),260,180);?>" alt="" />
<?php 						}  else { ?>
...
<?php 						} ?>
						...
						<div class="content_right content_right_inner"><span class="title_grey"><?php _e(PROVIDE_BY,'templatic');?></span>
								...
								if(get_post_meta($post->ID,'coupon_end_date_time',true) ) {  ?>
									<div class="deal_time_box">
										<div class="time_line"> </div>
											...
											?>
											<script>
											displayTZCountDown(setTZCountDown('<?php echo $tar_date[1]; ?>','<?php echo $tar_date[0]; ?>','<?php echo $tar_time[0]; ?>','<?php echo $timezone; ?>','<?php echo $exp_year[1]; ?>'),'countdowncontainer_<?php _e($post->ID,'templatic'); ?>','fr_<?php echo $post->ID; ?>','<?php echo $tardate1; ?>');
											</script>
									</div>
<?php 							} 
							}?>
...
<?php 						$post_categories = wp_get_post_terms($post->ID,'seller_category');
