<?php 
	...
	if(strstr($targetpage,'?')){
		if(strstr($targetpage,'?ptype')){
			...
			$target_querystr = "&amp;ptype=taxonomy_expired_deal_tab";
		}
	}else	{
		$target_querystr = "?ptype=taxonomy_expired_deal_tab";
	}
	...
	$all_dealcnt_sql = $wpdb->get_results("select distinct p.ID,p.* from $post_db_table_name p ,$postmeta_db_table_name pm where p.post_type = 'seller' and (pm.meta_key = 'is_expired' and pm.meta_value = '1') and p.ID = pm.post_id $sqlsql ORDER BY ID DESC  limit $strtlimit,$recordsperpage ");
