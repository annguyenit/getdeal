<?php
$deal_targetpage = "?ptype=taxonomy_all_deal_tab";
...
$dealcnt = "select p.* from $post_db_table_name p where p.post_type = 'seller' and p.post_status = 'publish' $sqlsql ORDER BY ID DESC ";
...
if (count($dealcnt_sql) > 0)
{
    ...
    foreach ($dealcnt_sql as $post)
    {
        ...
        <?php
        if ((strtotime(date("Y-m-d H:i:s")) >= strtotime($enddate1) && isset($enddate1) && $enddate1 != '') || get_post_meta($post->ID, 'no_of_coupon', true) == $sellsqlinfo)
        {
