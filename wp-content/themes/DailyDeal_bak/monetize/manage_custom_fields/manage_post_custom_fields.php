   ...
   foreach ($pt_metaboxes as $pt_id => $pt_metabox) {
    if($pt_metabox['type'] == 'text' OR $pt_metabox['type'] == 'select' OR $pt_metabox['type'] == 'radio' OR $pt_metabox['type'] == 'checkbox' OR $pt_metabox['type'] == 'textarea' OR $pt_metabox['type'] == 'upload' OR $pt_metabox['type'] == 'date' OR $pt_metabox['type'] == 'multicheckbox' OR $pt_metabox['type'] == 'texteditor')
            ...
            elseif ($pt_metabox['type'] == 'checkbox'){
				...
				$output .= "\t".'<div class="row">';
                $output .= "\t\t".'<p><label for="'.$pt_id.'">'.$pt_metabox['label'].'</label></p>'."\n";
                $output .= "\t\t".'<p class="value"><input type="checkbox" '.$checked.' class="pt_input_checkbox"  id="'.$pt_id.'" value="1" name="ptthemes_'. $pt_metabox["name"] .'" />'."\n";
      ...
      <div class="element">';
                $output .= "\t\t".'<p><label for="'.$pt_id.'">'.$pt_metabox['label'].'</label></p>'."\n";
                $output .= '<input type="text" class="'.$up_class.'"  id="ptthemes_'. $pt_metabox["name"] .'" name="ptthemes_'. $pt_metabox["name"] .'" value="'.$pt_metaboxvalue.'"/> <input id="upload_'.$pt_id.'" class="upload_button" type="button" value="Upload" rel="'.$pt_id.'" /><div class="screenshot" id="ptthemes_'. $pt_metabox["name"] .'_image">';
			...
			if($pt_metabox['type'] == 'geo_map'){
                ...
                $output .= "\t".'<div class="row">';
                $output .= "\t\t".'<p><label for="geo_address">'.__('Geo Address','templatic').'</label></p>'."\n";
                $output .= "\t\t".'<p><input size="100" class="pt_input_text" type="text" value="'.$geo_address.'" name="ptthemes_geo_address" id="geo_address"/></p>'."\n";
                $output .= "\t\t".'<p class="note">'.__('Please enter address. eg. : 230 Vine Street And locations throughout Old City, Philadelphia, PA 19106','templatic').'</p>'."\n";
                $output .= "\t".'</div>'."\n";
				
				$geo_latitude = get_post_meta($post->ID,'geo_latitude',true);
                $output .= "\t".'<div class="row">';
                $output .= "\t\t".'<p><label for="geo_latitude">'.__('Geo Latitude','templatic').'</label></p>'."\n";
                $output .= "\t\t".'<p><input size="100" class="pt_input_text" type="text" value="'.$geo_latitude.'" name="ptthemes_geo_latitude" id="geo_latitude"/></p>'."\n";
                $output .= "\t\t".'<p class="note">'.__('Enter Geo Latitude. eg. : 38.8225909761771','templatic').'</p>'."\n";
                $output .= "\t".'</div>'."\n";
				
				$geo_longitude = get_post_meta($post->ID,'geo_longitude',true);
                $output .= "\t".'<div class="row">';
                $output .= "\t\t".'<p><label for="geo_longitude">'.__('Geo Longitude','templatic').'</label></p>'."\n";
                $output .= "\t\t".'<p><input size="100" class="pt_input_text" type="text" value="'.$geo_longitude.'" name="ptthemes_geo_longitude" id="geo_longitude"/></p>'."\n";
                $output .= "\t\t".'<p class="note">'.__('Enter Geo Longitude. eg. : 12.65625','templatic').'</p>'."\n";
                $output .= "\t".'</div>'."\n";
